/*
 * CUSTOMIZED by Ross Anderson and Andrew Bradshaw
 *   Changes:
 *     Added lcd_image_draw_transparency, which reads the topleft pixel
 *     as the "transparency" marker. All other pixels in the image with that
 *     "transparency" color will NOT be drawn (i.e. drawn transparent). 
 *
 ...
 *
 * **********************************************************
 * THE TOPLEFT PIXEL IS THE COLOR WHICH WILL BE "TRANSPARENT"
 * **********************************************************
 */
void lcd_image_draw_transparency(lcd_image_t *img, Adafruit_ST7735 *tft,
		    uint16_t icol, uint16_t irow, 
		    uint16_t scol, uint16_t srow, 
		    uint16_t width, uint16_t height)
{
  ...
  if (scol > tft->width() || srow > tft->height()) {
	return;
  }
  ...
  for (uint16_t row=0; row < height; row++) {
    ...
    for (uint16_t col=0; col < width; col++) {
	  ...
	  if (!first_pixel_read) {
		first_pixel = pixel;
		first_pixel_read = true;
	  }

	  // If this is NOT the transparency pixel, then draw it.
	  if (pixel != first_pixel) {
		// pixel bytes in reverse order on card
		pixel = (pixel << 8) | (pixel >> 8);
		tft->drawPixel(scol + col, srow + row, pixel);
	  } 
    }
  }
